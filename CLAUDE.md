...
- `get_step_logs` sends raw HTTP requests (it needs the raw responses to tell a 0.96 server's missing route from ZenML's own 404s), but through the ZenML client's session (`get_zenml_client().zen_store.session`). So it shares the client's token and its `verify_ssl` and User-Agent settings, and it works however the client is connected (environment variables or `zenml login`). Like the client, it logs in only after a 401 (`store.authenticate()`), then retries once. `diagnose_zenml_setup` still logs in fresh with `get_access_token`, because checking the credentials is its job.
